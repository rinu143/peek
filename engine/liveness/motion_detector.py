...
class MotionParallaxDetector:
    ...
    def __init__(
        ...
        rigid_ratio_std_threshold: float = 0.003
    ):
    ...
    def update(
        ...
        # Modulates dynamically under natural 3D head pitch/yaw perspective
        ...
        # Calculate standard deviation of landmark coordinates and centroid across history
        ...
        if centroid_std < self.frozen_std_threshold and lm_std < (self.frozen_std_threshold * 0.8):
            # The face is unnaturally completely motionless (printed photo / frozen stream)
            return MotionLivenessResult(
                is_live=False,
                score=0.05,
                is_frozen=True,
        ...
        # --- Check 2: Rigid 2D Planar Motion (Waved Photo / Phone) ---
        # When an attacker moves a flat photo, the face translates in 2D, but the 
        # internal geometric ratio rho has virtually 0 variance because a 2D planar photo
        # lacks 3D depth parallax.
        ratio_arr = np.array(self._history_geom_ratios, dtype=np.float32)
        ratio_std = float(np.std(ratio_arr))

        # Substantial 2D translation (> 3.0 px) without internal geometric parallax
        if centroid_std > 3.0 and ratio_std < self.rigid_ratio_std_threshold:
            return MotionLivenessResult(
                is_live=False,
                score=0.15,
        ...
        # --- Check 3: Natural Micro-Movement & 3D Depth Score Calculation ---
        # Living humans exhibit subtle involuntary micro-tremors (0.4 - 3.5 px)
        # and natural 3D posture adjustments (ratio_std in [0.004, 0.15]).
        
        # Jitter score (reward natural human micro-variation between 0.3px and 4.0px)
        if centroid_std < 0.3:
            jitter_score = centroid_std / 0.3
        elif centroid_std <= 4.0:
            jitter_score = 1.0
        elif centroid_std <= 15.0:
            jitter_score = max(0.5, 1.0 - ((centroid_std - 4.0) / 22.0))
        else:
            jitter_score = 0.3  # Extreme motion/blur
